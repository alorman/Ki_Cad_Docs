...
- **Best Practice** I've found it's best to select all artwork (except `Edge.Cuts`) and change the fill to black and the stroke to none before exporting as SVG from Illustrator. This helped alleviate a lot of the errors above.
...
### MAJOR GOTCHAS
- **Note** For reasons I can't explain, the artwork brought through a (AI in mm) to (Rhino in mm) to (SVG in mm) (I know it's SVG mm because the numbers for overall size are correct in plain text) creates the wrong size footprint in KiCad. **Even when switching Illustrator to MM** some combination of viewbox, hard scale, or absolute numbers are wrong. I've had to scale all art my 0.6611 in Illustrator for the correct size to come through. `-- scale` in Gerboylze is meant for gerber output only. 
